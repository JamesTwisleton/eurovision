            ...
            {user && (
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 border-l border-muted-20 pl-4">
                <div className="flex items-center gap-1">
                  ...
                  {user.role === "HOST" && (
                    <Link
                      href={`/party/${user.partyKey}/members`}
                      title="Manage Party"
                      className={cn(
                        "flex h-9 w-9 items-center justify-center rounded-lg transition-all hover:bg-muted-10",
                        pathname === `/party/${user.partyKey}/members` ? "bg-neon-cyan/20 text-neon-cyan" : "text-muted-50"
                      )}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <span className="sr-only">Manage Party</span>
                    </Link>
                  )}
                </div>
                {pageName && (
